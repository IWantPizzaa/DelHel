...
Indicates the destination ICAO starts with one of the configured NAT prefixes (North/South America region warning).  
The prefixes are configured in `airports.json` under `_config.nat_arrival_icao_prefixes` (default: `B`, `C`, `K`, `M`, `S`, `T`).

##### `ARVL`

Caution, yellow color.  
Indicates the destination ICAO matches one of the configured arrival airports from `airports.json` under `_config.arvl_arrival_airports`.
...
#### Set airport linked/unlinked departures

`.delhel lfpg linked`  
`.delhel lfpg unlinked`  
`.delhel lfpo linked`  
`.delhel lfpo unlinked`

Controls whether departures for a configured airport are assigned using linked or unlinked procedures when processing flightplans.  
The selected linked/unlinked SID name is read from the `jet_dep`/`prop_dep` and `unlink_jet_dep`/`unlink_prop_dep` fields in `airports.json`.
This setting will be saved to the EuroScope settings upon exit.

#### Toggle LFPG CAS mode (north/south runway selection)

`.delhel lfpg cas`  
`.delhel lfpg nocas`

Enables or disables LFPG CAS mode, which selects a departure runway based on the SID's `north_south` classification and the runway `north_south` side from `airports.json`.  
Only active runways on the required side are considered. This setting will be saved to the EuroScope settings upon exit.
Enabling CAS will disable RM.

#### Toggle LFPG RM mode (minimum taxi north/south area)

`.delhel lfpg rm`  
`.delhel lfpg norm`

Enables or disables LFPG RM mode, which assigns north/south runways based on the aircraft spawn position inside the LFPG RM rectangles.  
Only active runways on the required side are considered. This setting will be saved to the EuroScope settings upon exit.
Enabling RM will disable CAS.
...
The `airports.json` file is a [JSON](https://www.json.org/) file containing a top-level object with:
- the departure airport ICAO code in uppercase as keys and `Airport` objects as values
- an optional reserved `_config` object for global settings

#### `_config` object (optional)

| Key                        | Type            | Description                                                                                              | Required |
| -------------------------- | --------------- | -------------------------------------------------------------------------------------------------------- | -------- |
| nat_arrival_icao_prefixes  | `array<string>` | ICAO first-letter prefixes that trigger the `NAT` warning on destination airport (e.g. `["B","C","K"]`) | No       |
| arvl_arrival_airports      | `array<string>` | Exact destination ICAO list that triggers the `ARVL` warning (e.g. `["KJFK","SBGL"]`)                  | No       |
